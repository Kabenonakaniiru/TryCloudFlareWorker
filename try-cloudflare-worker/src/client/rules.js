...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadTasks() {
        try {
            ...
            if (!resp.ok)
                throw new Error('マスタ一覧の取得に失敗しました');
    ...
    function openScopeModal(taskTitle, title, desc, onConfirm) {
        ...
        const futureRadio = document.querySelector('input[name="operation-scope"][value="future"]');
        if (futureRadio)
    ...
    if (modalConfirmBtn) {
        modalConfirmBtn.addEventListener('click', () => {
            const selectedScope = document.querySelector('input[name="operation-scope"]:checked');
    ...
    taskBtn.addEventListener('click', async () => {
        ...
        if (scheduleType === 'period') {
            ...
            const periodStyleEl = document.getElementById('task-period-style');
        ...
        const resetTimeEl = document.getElementById('task-reset-time');
        const missedBehaviorEl = document.getElementById('task-missed-behavior');
        ...
        if (editTaskId) {
            openScopeModal(title, 'ルール更新の反映範囲', 'このルールの変更に伴い、連動するGoogleカレンダーの予定をどう処理しますか？', async (scope) => {
                try {
                    ...
                    if (resp.ok) {
                        ...
                        alert('更新に失敗しました: ' + await resp.text());
                    }
                }
                catch (err) {
                    console.error(err);
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    alert('追加に失敗しました: ' + await resp.text());
                }
            }
            catch (err) {
                console.error(err);
    ...
    function editTask(id) {
        ...
        if (task.interval === 'period') {
            ...
            const endEl = document.getElementById('task-end');
            if (periodStyleEl)
                periodStyleEl.value = task.periodStyle || 'routine';
            if (startEl)
                startEl.value = task.startAt || '';
            if (endEl)
                endEl.value = task.endAt || '';
        }
    }
